...
func NewTruthTableGate(solutions [][]bool) (*TruthTableGate, error) {
	...
	inputs_len := int(math.Pow(2.0, float64(len(solutions))))
	if len(solutions) == 0 {
		return nil, ErrTTNoOutputs
	}
	...
	var rtn = &TruthTableGate{
		TruthTable: make([]TruthTableRow, inputs_len),
		Outputs:    make([]Line, solution_len),
		Inputs:     make([]Line, inputs_len),
	}
	for i, solution := range solutions {
		...
		rtn.TruthTable[i].BitsCount = inputs_len
	}
	return rtn, nil
}
...
func (g *TruthTableGate) Evaluate() []bool {
	var rtn []bool
	var index = 0
	for _, input_line := range g.Inputs {
		mask := 1 << len(g.Inputs)
...
func (g *TruthTableGate) ConnectInputProbe(probe LineProbe, index int, send_value ...bool) error {
...
func (g *TruthTableGate) DisconnectInputProbe(probe LineProbe, index int) error {
	if index < 0 || index >= len(g.Inputs) {
		return ErrGetInvalidLine
	}
...
func (g *TruthTableGate) ConnectOutputProbe(probe LineProbe, index int, send_value ...bool) error {
...
func (g *TruthTableGate) DisconnectOutputProbe(probe LineProbe, index int) error {
	if index < 0 || index >= len(g.Outputs) {
		return ErrGetInvalidLine
	}
